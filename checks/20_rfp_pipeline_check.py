...
from talentmatch.generation import RfpArtifactsPipeline
...
def run() -> int:
    ...
    try:
        pipeline = RfpArtifactsPipeline(settings)
        print_ok("RfpArtifactsPipeline(settings) succeeded")
    except Exception as exc:
        print_fail(f"RfpArtifactsPipeline(settings) failed: {exc}")
        return 1
    ...
    struct_dir = Path(settings.paths.rfp_struct_json_dir)
    md_dir = Path(settings.paths.rfp_markdown_dir)
    pdf_dir = Path(settings.paths.rfp_pdf_dir)
    ...
    if failures == 0:
        print_ok("RFP full pipeline checks passed")
        return 0

    print_fail(f"RFP full pipeline checks failed: {failures} issue(s)")
    return 1
